...
const BeautifulLoader: React.FC<BeautifulLoaderProps> = ({ darkMode }) => {
  return (
    <div className={`min-h-screen flex flex-col relative overflow-hidden ${
      darkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-pink-50 via-orange-50 to-pink-100'
    }`}>
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center relative z-10">
          ...
          <p className={`mt-2 text-lg font-medium ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              © 2024 HairCoaction. All rights reserved.
            </div>
            <div className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              Empowering hair care professionals worldwide
            </div>
          </div>
        </div>
      </footer>
